...
You will have noticed that running Cypress so far has opened an interactive browser which is not ideal for CI environments.
// To troubleshoot and debug failing tests in GitHub Actions, run Cypress headlessly using the `cypress run` command to view the test logs and error messages to identify the cause of the failure.:
...
Since Playwright is testing a real Next.js application, it requires the Next.js server to be running prior to starting Playwright. To troubleshoot and debug failing tests in GitHub Actions, view the test logs and error messages to identify the cause of the failure.
...
To troubleshoot and debug failing tests in GitHub Actions, you can view the test logs and error messages to identify the cause of the failure.

For more information on what to read next, we recommend:

    - #troubleshooting-failing-tests-in-github-actions
